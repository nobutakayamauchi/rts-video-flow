...
if [[ ! -x "${VENV_PYTHON}" ]]; then echo "[ERROR] Run ./scripts/setup.sh first." >&2; exit 1; fi
find_primary_video(){ find "${PROJECT_PATH}/camera" "${PROJECT_PATH}/screen" -maxdepth 1 -type f \( -iname '*.mov' -o -iname '*.mp4' -o -iname '*.m4v' -o -iname '*.webm' \) 2>/dev/null | sort | head -n 1; }
PRIMARY_VIDEO="$(find_primary_video || true)"; if [[ -z "${PRIMARY_VIDEO}" ]]; then echo "[ERROR] No camera or screen video found." >&2; exit 1; fi
...
"${VENV_PYTHON}" "${ROOT_DIR}/scripts/prepare_vlog_remotion.py" "${PROJECT_PATH}" --manifest "${OUTPUT_DIR}/manifest.json" --subtitles "${WORK_DIR}/subtitles.json" --remotion-dir "${ROOT_DIR}/remotion-project"
