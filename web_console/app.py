...
def extension_for(content_type: str | None, filename: str | None = None) -> str:
    ...
    if suffix in {".mp4", ".mov", ".m4v", ".webm", ".jpg", ".jpeg", ".png", ".webp", ".m4a", ".wav", ".ogg"}:
        return suffix
...
def load_plan(project: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        return default_plan(project.name)
    try:
...
def save_plan(project: Path, plan: dict[str, Any]) -> None:
    project.mkdir(parents=True, exist_ok=True)
    plan_path(project).write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding="utf-8")


def target_folder(project: Path, role: str) -> Path:
...
def project_status(project_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "outputExists": (OUTPUT_DIR / project.name / "vlog.mp4").is_file(),
    }
...
async def save_material(
    ...
    order = len(load_plan(project_path)["timeline"]) + 1
...
async def save_narration(
    ...
    project_path = project_dir(project)
    plan = load_plan(project_path)
    item = next((x for x in plan["timeline"] if x.get("id") == item_id), None)
    if not item:
...
def reorder(project: str = Form(...), ordered_ids: str = Form(...)) -> dict[str, Any]:
    ...
    if set(ids) != set(by_id):
        raise HTTPException(status_code=400, detail="Order does not match timeline items")
